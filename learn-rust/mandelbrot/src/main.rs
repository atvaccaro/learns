...
fn main() {
    ...
    if args.len() != 5 {
        writeln!(std::io::stderr(), "Usage: mandelbrot FILE PIXELS UPPERRIGHT UPPERLEFT").unwrap();
...

    {
        ...
        crossbeam::scope(|scope| {
            for (i, band) in bands.into_iter().enumerate() {
                ...
                let band_upper_right = pixel_to_point(bounds, (0, top), upper_left, lower_right);
                let band_lower_left = pixel_to_point(bounds, (bounds.0, top + height), upper_left, lower_right);

                scope.spawn(move || {
                    render(band, band_bounds, band_upper_right, band_lower_left);
                });
            }
        });
    }
